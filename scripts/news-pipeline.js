...
function systemPrompt(topics, nodes) {
  return `你是一位同时为「社会学、经济学、心理学」三种视角供稿的深度新闻编辑，尤其擅长把热点新闻改写成可直接用于高考议论文的标准论证段（符合新课标卷「基础等级」与「发展等级」评分要求）。
  ...
  "items": [
    {
      ...
      "essayQuote": "【高考议论文论证段，200–260 字，严格成 1 段】可直接作为高考议论文的一个完整论证段落使用，必须遵守「五步结构」：①观点句——用一句话亮明本段分论点（紧扣本条新闻的社科内核，如「极端天气常态化下，气候风险正在悄然重塑社会的分配逻辑」）；②阐释句——解释该观点与论题的关联；③材料句——引入本条新闻事实作为论据；④分析句——务必对材料做因果/假设/对比分析，点明「为何如此」「说明什么」，严禁只罗列现象；⑤结论句——回扣观点并适度升华。要求：论证有力、语言规范流畅、有文采但不浮夸、可当作文素材直接引用；严禁写成格言式金句、纯抒情口号或说明文段落；B 类新闻须显式写出其建构出的人/社会角度。",
...
- essayQuote 必须是一段 **200–260 字**的高考议论文论证段，严格遵守「观点句→阐释句→材料句→分析句→结论句」五步结构：观点句亮明分论点，材料句引入新闻事实，分析句必须对材料做因果/假设/对比分析（严禁只描述现象），结论句回扣升华；严禁写成一句格言式短句、纯抒情口号或说明文段落。B 类新闻须显式写出其建构出的人/社会角度，纯描述通报不得作为金句。
...
async function generateBatch(batchCandidates, { topics, nodes }, prevErrors) {
    const userMsg =
...
async function main() {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const raw = await generateBatch(batch, { topics, nodes }, attempt === 1 ? lastErr : null);
        // 归一化（纠偏回受控词表）
